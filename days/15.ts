...
const COMPUTER_DIRECTIONS: Record<number, number> = {
  [0]: 1,
  [1]: 4,
  [2]: 2,
  [3]: 3
...
const move = (pos: Point, dir: number) => {
  return { x: pos.x + DIRECTIONS[dir].x, y: pos.y + DIRECTIONS[dir].y }
}

const id = (pos: Point) => `${pos.x}_${pos.y}`
...
const create_map = () => {
  ...
  while (stack.length) {
    ...
    for (let i = 0; i < DIRECTIONS.length; i++) {
      const pos = move(current.pos, i)

      if (visited[id(pos)] === true) {
        continue
      }

      const result = it.next(COMPUTER_DIRECTIONS[i])

      if (result.value === GROUND) {
        stack.push({ pos, state: get_state(), dist: current.dist + 1 })

        // roll back move
        it.next()
        it.next(COMPUTER_DIRECTIONS[i + 2])
      } else if (result.value === OXYGEN) {
        goal = pos
        dist = current.dist + 1
      }
...
const render_map = (tiles: Point[], start: Point, goal: Point) => {
  const offsetx = Math.abs(Math.min(...tiles.map(p => p.x)))
  const offsety = Math.abs(Math.min(...tiles.map(p => p.y)))

  const width = Math.max(...tiles.map(p => p.x)) + offsetx + 1
  const height = Math.max(...tiles.map(p => p.y)) + offsety + 1

  const canvas = create_range(height).map(() => create_range(width).fill(' '))

  for (const tile of tiles) {
    canvas[tile.y + offsety][tile.x + offsetx] = '.'
  }

  canvas[start.y + offsety][start.x + offsetx] = 'S'
  canvas[goal.y + offsety][goal.x + offsetx] = 'G'
