...
const [a, b] = [first.split(','), second.split(',')]

type Point = [number, number]
type Line = [Point, Point]

const move = (pos: Point, change: string): Point => {
  const [dir, ...rest] = change.split('')
  const len = parseInt(rest.join(''), 10)

  switch (dir) {
    case 'R': return [pos[0] + len, pos[1]]
    case 'L': return [pos[0] - len, pos[1]]
    case 'U': return [pos[0], pos[1] + len]
    case 'D': return [pos[0], pos[1] - len]
    default: return [0, 0]
  }
}

const create_lines = (arr: string[]) => {
  const lines: Line[] = []
  let start: Point = [0, 0]

  for (let i = 0; i < arr.length; i++) {
    const end = move(start, arr[i])
...
const intersect = (a: Line, b: Line): Point | null => {
  const hor = a[0][1] === a[1][1] ? a : b
  const ver = a[0][0] === a[1][0] ? a : b

  const xmin = Math.min(hor[0][0], hor[1][0])
  const xmax = Math.max(hor[0][0], hor[1][0])

  if (ver[0][0] < xmin || ver[0][0] > xmax) return null

  const ymin = Math.min(ver[0][1], ver[1][1])
  const ymax = Math.max(ver[0][1], ver[1][1])

  if (hor[0][1] < ymin || hor[0][1] > ymax) return null

  return [ver[0][0], hor[0][1]]
}

const parallell = (a: Line, b: Line) => {
  return (a[0][0] === a[1][0] && b[0][0] === b[1][0])
    || (a[0][1] === a[1][1] && b[0][1] === b[1][1])
}

const find = (la: Line[], lb: Line[]) => {
  const points: Point[] = []
...
const manhattan = (p: Point) => Math.abs(p[0]) + Math.abs(p[1])

const vector = (a: Point, b: Point): Point => [Math.abs(a[0] - b[0]), Math.abs(a[1] - b[1])]

const walk = (lines: Line[], end: Point) => {
  let steps = 0

  for (let i = 0; i < lines.length; i++) {
    const line = lines[i]

    if (intersect(line, [end, end])) {
      steps += manhattan(vector(line[0], end))
      break
    } else {
      steps += manhattan(vector(line[0], line[1]))
    }
  }
...
const la = create_lines(a)
const lb = create_lines(b)

const points = find(la, lb).sort((a, b) => manhattan(a) - manhattan(b))

const part_one = () => console.log('What is the Manhattan distance from the central port to the closest intersection?', manhattan(points[0]))

const part_two = () => console.log('What is the fewest combined steps the wires must take to reach an intersection?', points.map(p => walk(la, p) + walk(lb, p)).sort()[0])
